#!/usr/bin/env python3
"""
Stage 14.3 — بصمة قاعدة البيانات + فحوص السلامة (قراءة فقط).

يُستخدم قبل/بعد تشغيل QA لمرحلة 14.3 لإثبات أن أي تغيير يخصّ بيانات الاختبار فقط،
وأن النظام غير مُدمِّر (لا حذف جماعي). يكتب /tmp/stage143-fingerprint-<mode>.json.
"""
...
TABLES = [
    "users", "roles", "permissions", "products", "categories", "brands", "specification_definitions",
    "product_specifications", "product_images", "inventory", "inventory_movements", "carts", "cart_items",
    "orders", "order_items", "payments", "idempotency_keys", "customer_verifications", "notifications",
    "notification_preferences", "notification_outbox", "audit_logs", "refresh_tokens", "user_roles",
    "role_permissions",
]
...
def sql(query: str) -> str:
    ...
    if result.returncode != 0:
        return "ERROR: " + result.stderr.strip().splitlines()[0][:100]
    return result.stdout.strip()


def fingerprint() -> dict:
    return {table: sql("select count(*) from %s" % table) for table in TABLES}
...
def integrity() -> dict:
    return {
        ...
        "duplicate_outbox": sql("select coalesce(sum(c - 1), 0) from (select count(*) c from notification_outbox group by event_type, aggregate_id having count(*) > 1) t"),
        ...
        "inventory_violations": sql("select count(*) from inventory where quantity < 0 or reserved_quantity < 0 or reserved_quantity > quantity"),
        "provider_leak": sql("select count(*) from payments where provider is not null or provider_payment_id is not null"),
        "fake_success": sql("select count(*) from payments where status = 'SUCCEEDED' and (reviewed_by is null or reviewed_at is null)"),
        "reviewed_without_reference": sql("select count(*) from payments where status = 'SUCCEEDED' and (transaction_reference is null or transaction_reference = '')"),
        "rejected_without_reason": sql("select count(*) from payments where status = 'FAILED' and (rejection_reason is null or rejection_reason = '')"),
        ...
        "stage143_test_customers": sql("select count(*) from users where phone in ('0955900100', '0955900160')"),
        "stage14_style_leftovers": sql("select count(*) from users where phone like 'stage14%' or phone like 'stage143%' or email ilike '%stage143%'"),
    }
...
def audit_secret_leaks() -> dict:
    return {
        ...
        "refresh_token_plain_in_db": sql("select count(*) from refresh_tokens where token_hash ilike 'ey%'"),
    }
...
def main() -> int:
    ...
    target = Path("/tmp/stage143-fingerprint-%s.json" % mode)
