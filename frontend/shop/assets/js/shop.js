...
(function (global) {
  ...
  var api = {
    ...
    /* السلايدر العام (Public) — ALW.api.get يفكّ غلاف {success,message,data} ويعيد data مباشرة */
    slider: function () {
      return ALW.api.get('/slider');
    },
  ...
  function stateBlock(kind, title, text, actions) {
    ...
    // STEP 3G: دور دلالي للحالة + إخفاء الأيقونة الزخرفية عن قارئ الشاشة
    var role = kind === 'error' ? 'alert' : 'status';
    return el('div', { class: 'state state--' + (kind === 'loading' ? 'compact' : kind), attrs: { role: role } }, [
      el('div', { class: 'state__icon', text: icon, attrs: { 'aria-hidden': 'true' } }),
  ...
  function loadingBlock(label) {
    // STEP 3G: الهياكل زخرفية (aria-hidden) والرسالة وحدها تُعلن كحالة
    return el('div', { class: 'shop-section', attrs: { 'aria-busy': 'true' } }, [
      el('div', { class: 'skeleton skeleton--title', attrs: { style: 'width:220px', 'aria-hidden': 'true' } }),
      el('div', { class: 'shop-grid', attrs: { 'aria-hidden': 'true' } }, [0, 1, 2, 3].map(function () {
        return el('div', { class: 'skeleton skeleton--card', attrs: { style: 'height:320px', 'aria-hidden': 'true' } });
      })),
      label ? el('p', { class: 'shop-note', text: label, attrs: { role: 'status' } }) : null,
    ]);
  }
