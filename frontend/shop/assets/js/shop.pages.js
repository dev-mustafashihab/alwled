...
(function (global) {
  ...
  function crumbs(items) {
    ...
    items.forEach(function (item, index) {
      ...
      var crumbAttrs = index === items.length - 1 ? { 'aria-current': 'page' } : null;   // (4B.2) الصفحة الحالية
      nodes.push(item.href ? el('a', { href: item.href, text: item.label, attrs: crumbAttrs }) : el('span', { text: item.label, attrs: crumbAttrs }));
    });
  ...
  function home(view) {
    ...
    return Promise.all([
      ...
      /* السلايدر: فشل الـAPI ⇒ مصفوفة فارغة ⇒ لا قسم إطلاقًا (الصفحة تكمل طبيعيًا) */
      shop.api.slider().catch(function () { return []; }),
    ]).then(function (results) {
      /* حارس race: إن غادر المستخدم Home قبل اكتمال الطلبات لا نكتب في view قديمة */
      if (view.isConnected === false) return null;
      var products = results[0] || {};
      var featured = (results[1] && results[1].items) || [];
      var categories = (results[2] && results[2].items) || [];
      /* شرائح السلايدر: ALW.api.get يعيد data مباشرة (مصفوفة) — ندعم الشكلين احتياطًا */
      var sliderSlides = Array.isArray(results[4]) ? results[4] : ((results[4] && results[4].data) || []);
      ...
      if (typeof homeSliderCleanup === 'function') homeSliderCleanup(); // لا تسريب مؤقّتات/مستمعات عند العودة
      shop.clear(view);

      /* 1 — السلايدر الحقيقي: مصدره الوحيد GET /slider (لا رسائل ولا شرائح مُختلقة)
         صفر شرائح أو فشل الـAPI ⇒ لا قسم إطلاقًا (بلا مساحة محجوزة وبلا انهيار) */
      var sliderNode = homeSlider(sliderSlides);
      if (sliderNode) view.appendChild(sliderNode);
  ...
  var homeSliderCleanup = null; // تنظيف صريح للمؤقّت/المستمعات عند مغادرة Home
  ...
  function homeTicker(messages) {
    ...
    // نسخ مكرّرة للّف السلس بلا فراغ: 6 نسخ ⇒ التحريك بمقدار نسخة واحدة يكفي لتغطية أي عرض
    // (كل النسخ مخفية عن قارئ الشاشة؛ النص المقروء يأتي من القائمة المخفية أدناه مرة واحدة)
    var seed = track.cloneNode(true);   // نسخة أصلية ثابتة — وإلا تنسخ الحلقة مسارًا متناميًا
    for (var c = 0; c < 5; c++) {
    ...
    return el('div', {
      ...
      el('div', { class: 'shop-home-ticker__viewport', attrs: { 'aria-hidden': 'true' } }, [track]),
  ...
  /* محرّك السلايدر الوحيد (تطوير جراحي للمحرك القائم) — مصدره GET /slider حصرًا.
     لا بيانات مُختلقة · لا crop · لا distortion · controls/autoplay/swipe عند >1 فقط. */
  function homeSlider(slides) {
    var list = (slides || []).filter(function (s) { return s && s.desktopImageUrl; });
    if (!list.length) return null;               // صفر شرائح ⇒ لا قسم إطلاقًا (بلا مساحة محجوزة)
    ...
    function picture(slide, i) {
      var img = el('img', {
        class: 'shop-home-slide__img',
        attrs: {
      ...
      if (slide.mobileImageUrl) {
        kids.push(el('source', { attrs: { media: '(max-width: 768px)', srcset: slide.mobileImageUrl } }));
      }
      kids.push(img);
      return el('picture', {}, kids);
    }

    function overlay(slide) {
      var has = slide.title || slide.subtitle || slide.ctaLabel;
      if (!has) return null;                      // لا overlay أصلًا عند غياب المحتوى
      return el('div', { class: 'shop-home-slide__body' }, [
        slide.title ? el('h2', { class: 'shop-home-slide__title', text: slide.title }) : null,
        slide.subtitle ? el('p', { class: 'shop-home-slide__text', text: slide.subtitle }) : null,
        slide.ctaLabel ? el('span', { class: 'shop-home-slide__cta', text: slide.ctaLabel }) : null,
      ]);
    }

    list.forEach(function (slide, i) {
      var inner = [picture(slide, i), overlay(slide)];
      var isLink = !!slide.linkUrl;
      var external = isLink && /^https?:\/\//i.test(slide.linkUrl);
      var node = el(isLink ? 'a' : 'div', isLink
        ? {
            class: 'shop-home-slide__link',
            attrs: {
              href: slide.linkUrl,
              'aria-label': slide.altText || undefined,
              target: external ? '_blank' : undefined,
              rel: external ? 'noopener noreferrer' : undefined,
            },
          }
        : { class: 'shop-home-slide__link' }, inner);
      track.appendChild(el('article', {
        class: 'shop-home-slide',
    ...
    function syncA11y() {
      Array.prototype.slice.call(track.children).forEach(function (node, k) {
        ...
        try { node.inert = !active; } catch (e) { /* متصفح قديم */ }
        Array.prototype.slice.call(node.querySelectorAll('a[href], button')).forEach(function (el2) {
          if (active) el2.removeAttribute('tabindex'); else el2.setAttribute('tabindex', '-1');
        });
      });
    ...
    function render(i, manual) {
      index = (i + list.length) % list.length;
      track.style.transform = 'translateX(' + (index * 100) + '%)';
      syncA11y();
      if (manual) { stop(); start(); }             // التنقّل اليدوي يعيد إدارة المؤقّت (بلا مؤقّتات مكرّرة)
    }
    function stop() {
    ...
    function start() {
      stop();
      if (!multiple) return;                       // شريحة واحدة ⇒ لا autoplay إطلاقًا
    ...
    function onFocusOut() { paused = false; }
    var startX = null;
    function onTouchStart(event) { startX = event.touches[0].clientX; paused = true; }
    function onTouchEnd(event) {
      if (startX === null) return;
      var dx = event.changedTouches[0].clientX - startX;
      startX = null; paused = false;
      if (Math.abs(dx) > 40) render(dx < 0 ? index + 1 : index - 1, true);   // RTL
    }
    function onHashChange() { if (!(global.document || document).body.contains(root)) cleanup(); }

    if (multiple) {
      var prev = root.querySelector('.shop-home-slider__arrow--prev');
      var next = root.querySelector('.shop-home-slider__arrow--next');
      if (prev) prev.addEventListener('click', function () { render(index - 1, true); });
      if (next) next.addEventListener('click', function () { render(index + 1, true); });
    ...
    function cleanup() {
      ...
      if (homeSliderCleanup === cleanup) homeSliderCleanup = null;
    }
    homeSliderCleanup = cleanup;                   // يُنظَّف صريحًا قبل أي رسم جديد لـHome
  ...
  function product(view, ctx) {
    ...
    return shop.api.product(id).then(function (payload) {
      ...
      /* (4B.2) صورة مكسورة ⇒ بديل داخل المعرض (المعالج على عقدة المعرض فقط، بلا مساس imageNode المشترك) */
      var stageImg = stage.querySelector('img');
      if (stageImg) {
        stageImg.addEventListener('error', function () {
          if (!stage.contains(stageImg)) return;
          stage.removeChild(stageImg);
          stage.appendChild(el('div', { class: 'shop-card__placeholder', html: productIconSvg() }));
        });
      }
      ...
      var buy = el('div', { class: 'shop-buy' }, [
        ...
        /* (4B.2) صف حالة واحد: التوفّر فقط — العلامة/التصنيف/SKU تظهر مرة واحدة في سطر الميتا أعلاه
           (كانت مكرّرة 4 خلايا = 152px تدفع CTA إلى حدّ الطية). */
      ...
      /* (4B.2) وصف المنتج: description ثم shortDescription (حقل متاح فعلاً) — بلا اختلاق محتوى */
      var description = item.description || item.shortDescription;
      if (description) {
        view.appendChild(el('section', { class: 'shop-panel' }, [
          el('h2', { class: 'shop-panel__title', text: 'وصف المنتج' }),
          el('div', { class: 'shop-prose', text: description }),
        ]));
      }
      ...
      if (relatedCategoryId) {
        shop.api.products({ categoryId: relatedCategoryId, limit: 8, includeInventory: true }).then(function (payload) {
          ...
          view.appendChild(el('section', { class: 'shop-section shop-section--related' }, [
          ...
          /* (4B.2) قسم اختياري لا يُعطّل الصفحة — لكن بلا فشل صامت: حالة مصغّرة بلا طلب إضافي */
          if (view.querySelector('.shop-section--related-error')) return;
          view.appendChild(el('section', { class: 'shop-section shop-section--related-error' }, [
            el('p', { class: 'shop-note', text: 'تعذّر تحميل المنتجات المشابهة.', attrs: { role: 'status' } }),
          ]));
        });
      }
