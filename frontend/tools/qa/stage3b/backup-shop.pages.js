...
(function (global) {
  ...
  function renderProducts(view, state, payload) {
    ...
    // فلترة التوفر/الخصم على ما يعرضه الـAPI فعلاً (لا يفترض بيانات غير موجودة)
    if (filters.inStock === '1') items = items.filter(function (p) { return p.inStock !== false; });
    if (filters.offers === '1') items = items.filter(function (p) { return p.hasDiscount === true; });
    var shownTotal = (filters.inStock === '1' || filters.offers === '1') ? items.length : (meta.total || items.length);
    ...
    function apply(next) {
      ...
      if (next && next.page === undefined && !next.keepPage) merged.page = 1;
      var clean = {};
    ...
    var toolbar = el('div', { class: 'shop-toolbar', id: 'shop-toolbar', attrs: { role: 'search' } }, [
      ...
      el('div', { class: 'shop-toolbar__field shop-toolbar__field--filter' }, [
        el('span', { class: 'shop-toolbar__label', text: 'التوفر' }),
        el('label', { class: 'shop-check', for: 'shop-filter-instock' }, [
          el('input', {
            type: 'checkbox', checked: filters.inStock === '1' ? 'checked' : null,
            attrs: { id: 'shop-filter-instock' },
          }),
          el('span', { text: 'المتوفر فقط' }),
        ]),
      ]),
      el('div', { class: 'shop-toolbar__field shop-toolbar__field--filter' }, [
      ...
      el('div', { class: 'shop-toolbar__count', text: shownTotal + ' منتج' }),
    ]);
    ...
    var filtersToggle = el('button', {
      ...
      attrs: { 'aria-expanded': filtersOpen ? 'true' : 'false', 'aria-controls': 'shop-toolbar' },
    }, [
    ...
    filtersToggle.addEventListener('click', function () {
      ...
      if (open) {
        var first = toolbar.querySelector('input, select');
        if (first && window.innerWidth > 768 && first.focus) first.focus();
      }
    });
    ...
    var inStockBox = toolbar.querySelector('#shop-filter-instock');
    ...
    if (inStockBox) inStockBox.addEventListener('change', function () { apply({ inStock: inStockBox.checked ? '1' : '' }); });
    ...
    var sheetDone = el('button', {
      ...
      onclick: function () { filtersWrap.classList.remove('is-open'); toolbar.classList.remove('is-filters-open'); },
    });
    var sheetClear = el('button', {
      class: 'btn btn--ghost shop-filters__clear', type: 'button', text: 'تفريغ الفلاتر',
      onclick: function () { global.location.hash = shop.buildHash({ path: '/products' }); },
    });
    var scrim = el('div', {
    ...
    filtersWrap.appendChild(el('div', { class: 'shop-filters__sheet' }, [
      el('div', { class: 'shop-filters__sheet-head' }, [
        el('strong', { text: 'الفلاتر والترتيب' }),
        el('button', {
  ...
  function product(view, ctx) {
    ...
    return shop.api.product(id).then(function (payload) {
      ...
      var addButton = el('button', {
        class: 'btn btn--primary btn--lg', type: 'button', text: 'أضف إلى السلة',
        attrs: { id: 'shop-add-to-cart', 'data-product-id': String(model.id) },
        onclick: function () { ALW.shopAddToCart(addButton, model, qty); },
      });
      ...
      var buy = el('div', { class: 'shop-buy' }, [
        ...
        el('p', { class: 'shop-note', text: 'الأسعار معروضة كما هي في نظام المتجر. الدفع عبر شام كاش بتحويل يدوي مع مراجعة من فريق المتجر.' }),
      ]);
